"""
corrections_reco_anomalies.py — la TABLE de `fix_reco_anomalies`.

POURQUOI CE FICHIER EST SEPARE
------------------------------
`fix_reco_anomalies.py` pesait 1533 lignes, trois fois la limite du projet.
La mesure a montre que 1311 d'entre elles etaient de la DONNEE curee et 222 de
la logique : le fichier n'etait pas trop complexe, il portait un jeu de donnees
qui grossit a chaque correction verifiee.

Separer les deux rend la logique lisible d'un seul tenant, et met cette table
la ou elle appartient. Ce module ne contient AUCUN code executable : que des
donnees et les commentaires qui les justifient.

POURQUOI PAS DU JSON
--------------------
Chaque entree porte la citation ou le fait qui a tranche la decision — souvent
plusieurs lignes de commentaire. JSON ne sait pas les garder, et sans elles
personne ne peut rejuger une correction. Le format Python est ici le seul qui
preserve le POURQUOI.
"""
from __future__ import annotations

from typing import Any

__all__ = ["CLES_EFFET", "CORRECTIONS"]
