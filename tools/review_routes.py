...
_SAVE_ACTIONS: frozenset[str] = SAVE_ACTIONS
...
class Handler(MergeRoutesMixin, RecoCrudRoutesMixin, TableRoutesMixin,
    ...
    def _apply_save_action(reco: dict, action: str, recommended: str,
                           reco_id: str) -> None:
        """#16 review — décision HUMAINE : mute `reco` selon `action`.

        La mutation elle-même vit dans `review_actions.apply_review_action`,
        partagée avec `apply_verdicts.py` (verdicts d'agents).
        """
        ...
        reco.setdefault("agentReview", {})["reviewedByHuman"] = True
        apply_review_action(reco, action, recommended, reco_id)
